...
function HubNode({ node, index, count, scrollYProgress, radius }) {
  ...
  return (
    <motion.div
      className="absolute -translate-x-1/2 -translate-y-1/2 w-10 h-10 sm:w-[52px] sm:h-[52px] rounded-full bg-white/[0.06] border border-white/10 backdrop-blur-md flex items-center justify-center"
      style={{ left, top, opacity, scale, boxShadow: `0 0 18px ${node.color}30` }}
    >
  ...
  return (
    <div className="relative mb-4">
      <div ref={ref} className="relative w-full max-w-[560px] aspect-square mx-auto">
        ...
        <motion.div
          className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center w-[76px] h-[76px] sm:w-[96px] sm:h-[96px] rounded-full bg-grad-main border border-white/20 z-[2]"
          style={{
            scale: centerScale,
